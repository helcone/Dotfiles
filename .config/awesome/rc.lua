...
-- defaul themes are zenburn, gtk, sky, xresources, blackarch

-- beautiful.init(gears.filesystem.get_themes_dir() .. "xresources/theme.lua") -- Defaults
beautiful.init("/home/user/.config/awesome/xresources.lua")
...
-- Personalized Keys 
   ...
   --]]
    ...
    awful.util.spawn("torbrowser-launcher") end, 
	      {description = "run tor browser", group = "Applications"}),
    ...
    awful.key({ modkey }, "0", function() volume_widget:toggle() end,
                {description = "Toggle Device", group = "Multimedia"}),
    ---[[ Ranger 
...
--]]
    ...
    awful.util.spawn("joplin-desktop") end, 
...
clientkeys = gears.table.join(
  ...
  -- working toggle titlebar
    ...
    awful.key({ modkey, "Shift"   }, "t",      function (c) c.sticky = not c.sticky            end,
              {description = "toggle keep on stick", group = "client"}),
    awful.key({ modkey,           }, "n",
        function (c)
